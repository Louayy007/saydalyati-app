    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="ml-64 min-h-screen">
        ...
        <header className="bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between sticky top-0 z-20">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold bg-teal-100 text-teal-700 px-2 py-0.5 rounded-full">Admin</span>
            </div>
            <p className="text-sm text-gray-400 mt-0.5">Vue globale de la plateforme, {displayName}</p>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/admin')}
              className="flex items-center gap-2 bg-teal-500 hover:bg-teal-600 text-white text-sm font-semibold px-4 py-2 rounded-xl transition">
            ...
            <button onClick={() => navigate('/profile')}
              className="flex items-center gap-2.5 rounded-xl px-2 py-1 hover:bg-gray-50 transition">
        ...
        <div className="px-8 py-6 space-y-6">
          ...
          {/* ── Stats globales ── */}
          <div>
            <h2 className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-3">Utilisateurs</h2>
          ...
          <div>
            <h2 className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-3">Annonces & Échanges</h2>
            <div className="grid grid-cols-4 gap-4">
            ...
            <div className="col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-semibold text-gray-800 text-sm">Comptes en attente</h2>
                    <p className="text-xs text-gray-400">Nouvelles inscriptions à valider</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <Link to="/admin" className="text-sm text-teal-500 hover:text-teal-700 font-medium transition">
                    Voir tout →
                  </Link>
                </div>
              </div>
                ...
                {!loading && pendingUsers.length > 0 && (
                  <div className="space-y-3">
                    {pendingUsers.slice(0, 4).map(u => (
                      <div key={u.id} className="flex items-center justify-between border border-gray-100 rounded-xl p-4 hover:bg-gray-50 transition">
                        <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <button onClick={() => handleApproval(u.id, 'approved')}
                            className="px-3 py-1.5 rounded-lg bg-teal-500 hover:bg-teal-600 text-white text-xs font-semibold transition">
                            Approuver
                          </button>
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <div className="space-y-2">
                  <button onClick={() => navigate('/admin')}
                    className="flex items-center gap-3 w-full bg-teal-500 hover:bg-teal-600 text-white text-sm font-medium px-4 py-3 rounded-xl transition">
                  ...
                  <button onClick={() => navigate('/marketplace')}
                    className="flex items-center gap-3 w-full bg-gray-50 hover:bg-gray-100 text-gray-700 text-sm font-medium px-4 py-3 rounded-xl transition border border-gray-200">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4 text-teal-500 shrink-0">
              ...
              {!loading && analytics && (
                <div className="bg-gradient-to-br from-teal-500 to-teal-700 rounded-2xl p-5 text-white shadow-md">
                  <p className="text-sm font-medium text-teal-100 mb-1">Taux d'approbation</p>
                  {analytics.totalUsers > 0 ? (
                    ...
                    <p className="text-teal-200 text-sm mt-1">Aucun utilisateur encore</p>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-semibold text-gray-800 text-sm">Échanges récents (plateforme)</h2>
                  <p className="text-xs text-gray-400">Les 5 dernières demandes d'échange</p>
                </div>
              </div>
            </div>
            {loading ? (
              ...
              <div className="p-6 text-center text-sm text-gray-400">Aucun échange pour l'instant</div>
            ) : (
                    ...
                    {recentRequests.map(req => {
                      ...
                      return (
                        <tr key={req.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                          ...
                          <td className="py-3 px-4 text-sm text-gray-500">{req.listing?.owner?.establishmentName || req.listing?.owner?.email || '—'}</td>
                          <td className="py-3 px-4 text-sm text-gray-500">{req.requester?.establishmentName || req.requester?.email || '—'}</td>
                          <td className="py-3 px-4">
                            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${statusStyle}`}>{statusLabel}</span>
                          </td>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="ml-64 min-h-screen">
        ...
        <header className="bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between sticky top-0 z-20">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Tableau de bord</h1>
            <p className="text-sm text-gray-400 mt-0.5">Bonjour, {displayName} 👋</p>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/exchange-requests')}
              className="relative w-9 h-9 rounded-xl bg-gray-50 hover:bg-gray-100 flex items-center justify-center transition border border-gray-200">
            ...
            <div className="col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-semibold text-gray-800 text-sm">Annonces urgentes</h2>
                    <p className="text-xs text-gray-400">Médicaments nécessitant une action rapide</p>
                  </div>
                </div>
                <Link to="/marketplace" className="text-sm text-teal-500 hover:text-teal-700 font-medium">Voir tout →</Link>
              </div>
